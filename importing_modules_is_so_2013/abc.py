...
class board:
    ...
    def additem(self, item, coordstuple):
        self.items[coordstuple]+=item

class item(Turtle):

    visited=False
    def __init__(self, shape=_CFG["shape"], undobuffersize=_CFG["undobuffersize"],
                 visible=_CFG["visible"]):
        self.visited=False
        if Turtle._screen is None:
            Turtle._screen = Screen()
    ...
    def start(self, tar):
        if not self.visited:
            #specific actions for items inheriting from this, if 1time - visited=True
            self.visible=0
...
def forwarder(obj, c):
    ...
    yield 1
    obj.shapesize(0.5)
    if link[0]==obj.parent:
        enemy=1
    else:
        enemy=0
    lolcheck = lambda x: obj.distance(link[enemy].current) < 10 
##    if obj.distance(link[enemy].current) < 10:
##        print print 'BOOM!\n{} wins'.format(enemy)
##        time.sleep(5)
    if lolcheck(0):
        print 'BOOM'
...
class CustomTurtleTemplate(Turtle):
    score=0
    parent=None
    visited=set([])
    current=(0,0)
    ...
    def forwardd(self, n):
        ...
        self.declare((self.xcor(),self.ycor()))
    def addr1(self,postuple):
        one, two = int(postuple[0]), int(postuple[1])
        self.visited.add((one,two))
        self.visited.add((one-1,two))
        self.visited.add((one+1,two))
        self.visited.add((one,two-1))
        self.visited.add((one,two+1))
        self.visited.add((one-1,two+1))
        self.visited.add((one+1,two+1))
        self.visited.add((one-1,two-1))
        self.visited.add((one+1,two-1))
        
    def declare(self, postuple):
        self.addr1(postuple)
        self.current=(int(postuple[0]), int(postuple[1]))
    ...
    def radar(self):
        if link[0]==self:
            ...
            enemy=0
        dst = self.distance(link[enemy].xcor(), link[enemy].ycor())
        dirr=(360-self.heading()) % 360

        sx, sy, ox, oy = self.xcor(), self.ycor(),link[enemy].xcor(), link[enemy].ycor()
        angle=math.atan2(abs(sx-ox), abs(sy-oy))
        ...
        headingangle=angle-dirr
        return headingangle, dst
    ...
    def start(self):
        self.setpos(self.loc[0], self.loc[1])
        self.pendown()
        #specific implementations code here..


class t1(CustomTurtleTemplate):

    def start(self):
        self.color('blue')
        #you can choose this!
        self.setstart(100,100)
        
        
        CustomTurtleTemplate.start(self)
        #YOUR CODE BELOW!!!
        while 1:
            ...
            while (angle % 360 > 5) and (angle % 360 < 355):
                if angle>0:
                    self.r()
...
p1='player1name'
p2='player2name'
iterables+=set([lol, lol2])
current=time.time()
while time.time()-current < 10000000000000000:
...
##        a.score += 5
##        b.score += 5
##        print 'both players gain 5 points!'
##    elif a.current in b.visited:
##        b.score += 3
##        print '{} gains 3 points!'.format(p2)
##    elif b.current in a.visited:
##        a.score += 3
##        print '{} gains 3 points!'.format(p1)
    
print 'terrain covered bonus: \n{}: {}\n{}: {}'.format(p1, len(a.visited),p1, len(b.visited))

print '1score: {}'.format(a.score)
print '1score: {}'.format(b.score)
